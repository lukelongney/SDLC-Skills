...
{id:"s8",name:"S8 — Security Assessment",
mode:"agentic",
interactionPattern:"Autonomous after framework confirmation. User confirms or adjusts framework set based on project classification, then agent runs assessment independently. Produces findings report and offers Jira ticket creation (user confirms before creation).",
summary:"Independent security assessment of design artifacts against industry frameworks. Framework tiering by project classification (Standard/Elevated/Critical) with user override. Spot checks high-priority threats have Jira stories. Distinct from S5 — S8 verifies design, doesn't redesign.",
inputs:["Solution Design (from S4)","Threat Model (from S5)","Jira Scaffolding (from S7)","Feasibility (from S2) for project classification","Framework tiers: Standard (Top 10 + ASVS L1), Elevated (+ASVS L2 + SOC 2), Critical (+ASVS L3 + ISO 27001 + MITRE ATT&CK)"],
outputs:[{name:"Security Assessment Report",fields:["Framework coverage per selected framework","Findings by severity (Critical/High/Medium/Low)","OWASP Top 10 2025 coverage matrix","OWASP ASVS coverage by section","MITRE ATT&CK detection design coverage (if Critical tier)","Jira spot check results (3-5 high-priority threats)"]},{name:"Verdict",fields:["Approved — no Critical or High findings","Approved with Conditions — High findings documented with plan","Not Approved — Critical findings must be resolved"]},{name:"Optional Jira Tickets",fields:["Defect tickets for gaps (user confirms before creation)","Uses same Jira integration as S7"]}],
mcpIntegrations:[{name:"Confluence MCP",direction:"read + write",operations:["READ: Security standards and policies","READ: Previous assessments for consistency","WRITE: Publish security assessment report"]},{name:"Jira MCP",direction:"read + write",operations:["READ: S7 stories for spot check (THR-xxx labels)","WRITE: Create defect tickets for gaps (with permission)"]}],
gate:{type:"Security team review",criteria:"Approved, Approved with Conditions, or Not Approved. High findings allow conditional proceed. Critical findings block until resolved."},
designNotes:"Design-phase assessment only — no codebase scanning (that's repo-level security). Tiered frameworks prevent over-engineering simple projects. Spot check for Jira coverage avoids noise for brownfield work. User controls Jira ticket creation."},

{id:"s9",name:"S9 — Operational Readiness & Handoff",
mode:"agentic",
interactionPattern:"Fully autonomous consolidation. Agent reads all artifacts (S1-S8), extracts highlights, assesses change impact and operational readiness, compiles audience-specific handoff document. Informs delivery, does not gate.",
summary:"Consolidates design phase into handoff package for delivery. Assesses change impact (technical + organizational) and operational readiness indicators. Produces audience-specific sections (exec, PM, tech, ops, support). Soft readiness indicators inform delivery planning — not a blocking gate.",
inputs:["All outputs from S1–S8","Extracts: scope (S1), ROM/risks (S2), requirements (S3), architecture/ADRs (S4), threats (S5), validation (S6), Jira structure (S7), security verdict (S8)"],
outputs:[{name:"Change Impact Assessment",fields:["Technical: systems affected, new components, integrations, infrastructure","Organizational: teams affected, training needs, process changes, support model","Impact rating: Low / Medium / High"]},{name:"Operational Readiness Indicators",fields:["Monitoring — Complete / Partial / Gap","Alerting — thresholds defined?","Logging — approach designed?","Incident Response — runbook approach?","Rollback — procedure designed?","Support Model — L1/L2/L3 defined?","Escalation Paths — matrix identified?","SLA Implications — targets defined?"]},{name:"Audience-Specific Sections",fields:["Executive Summary (sponsors/stakeholders)","Delivery Overview (project managers)","Technical Handoff (dev leads/architects)","Operational Handoff (ops/SRE)","Support Handoff (service desk/L2)"]},{name:"Confluence-Ready Output",fields:["Wiki markup version with expand/status/panel macros","Ready for direct import via wiki markup macro"]}],
mcpIntegrations:[{name:"Confluence MCP",direction:"read + write",operations:["READ: All project artifacts for consolidation","WRITE: Publish handoff document","WRITE: Create Confluence-formatted version"]},{name:"Jira MCP",direction:"read",operations:["READ: Epic/story counts from S7","READ: Verify structure for handoff summary"]}],
gate:{type:"Informational — delivery decides",criteria:"Not a gate. Readiness indicators inform delivery planning. Gaps noted for sprint 0. Delivery team decides how to proceed."},
designNotes:"Handoff package, not a gate. Consolidates highlights so delivery gets the full picture in one read. Change impact helps delivery plan resources and communication. Readiness indicators flag what needs attention without blocking."}
